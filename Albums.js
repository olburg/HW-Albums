...
const makeUserFromObject = (user) => {
  ...
  man.innerText = user.name;
  div.appendChild(man);

  const twoName = document.createElement("h2");
  twoName.innerText = user.username;
  div.appendChild(twoName);

  const email = document.createElement("p");
  email.innerText = user.email;
  div.appendChild(email);

  div.appendChild(getAlbums(user.id));

  return div;
};

const getAlbums = (manId) => {
  const alb = document.createElement("div");
  const fined = albums.filter((album) => album.userId === manId);
  if (!fined.length) {
    alb.innerText = "No Albums";
  ...
  fined.forEach((album) => {
    ...
    titleA.innerText = album.title;
    div.appendChild(titleA);
    div.appendChild(getFoto(album.id));
    alb.appendChild(div);
  });
  return alb;
};

const getFoto = (fotoId) => {
  const div = document.createElement("div");
  const finedFoto = photos.filter((photo) => photo.albumId === fotoId);

  if (!finedFoto.length) {
    div.innerText = "No Photos";
    return div;
  }
  finedFoto.forEach((photo) => {
    const fotoBlock = document.createElement("div");
    const foto = document.createElement("img");
    foto.src = photo.thumbnailUrl;
    fotoBlock.appendChild(foto);
    div.appendChild(fotoBlock);
  });
